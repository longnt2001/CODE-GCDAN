...
def run_transform(batches, mode, model, opt, criterion, graph = None):
    ...
    for i, batch in enumerate(batches):
        print('batch:'+str(i))
        opt.zero_grad()
        loc = batch['src_loc'][0:1]
        st = batch['src_st'][0:1]
        ed = batch['src_ed'][0:1]
        tgt_loc = batch['tgt_loc'][0:1]
        tgt_st = batch['tgt_st'][0:1]
        tgt_ed = batch['tgt_ed'][0:1]
        tgt_y = batch['tgt_y'][0:1]
        tgt_len = batch['tgt_len'][0:1]
        uid = batch['uid'][0:1]
        loss = None
        tot_tgt = 0
        break
...
def run(args):
    ...
    for epoch in range(parameters.epoch):
        ...
        model.eval()
        test_loss, acc = run_transform(test_batches[0:1], 'test', model, optimizer, criterion)
        ...
        model.load_state_dict(torch.load('D:\\GCDAN-master\\results\checkpoint\\ep_4.m'))
        print('load epoch={} model state')
